...
def cli_args():
    ...
    parser.add_argument('--dns-alts', action='store_true',
                        help='try alternative permutated subdomains, based on finded')
    parser.add_argument('--workflow-olds', action='store_true',
                        help='httpprobe old subs to find changes, else check only new subdomains')
...
def sites_workflow(domains, httpx_threads=1):
    ...
    up_fields = ["url", "scheme", "port", "hash", "a", "cnames", "input", "location", "title", "webserver",
                 "content_type", "method", "host", "content_length", "words", "lines", "chain_status_codes",
                 "status_code", "tls",
    ...
    notify_by_weight(sites_new, "probe(s)",
                     lambda x: f"{x['url']} [{x['status_code']}] [{x.get('title', '')}]{x['juicy_info']}")
    ...
    # new nuclei hits
    up_fields = ["template-id", "info", "type", "matcher-name", "host", "matched-at", "meta", "extracted-results",
...
def passive_workflow(all_http_probes):
    for scan_type in ['passive']:
        ...
        # new nuclei hits
        up_fields = ["template-id", "info", "type", "matcher-name", "host", "port", "path", "matched-at", "meta",
                     "extracted-results", "scope"]
        index_fields = ["template-id", "matcher-name", "host"]
        nuclei_hits_new = db_get_modified(passive_results, db['nuclei_passive_hits'], index_fields, up_fields,
                                          compare.nuclei_hit)
        nuclei_hits_new = list(nuclei_hits_new)
        nuclei_notify(
            nuclei_hits_new,
            lambda
                x: f'{x["scope"]}: {x["host"]} [{x["info"]["severity"]}] {x["template-id"]} {x.get("matcher-name", "")} {x.get("extracted-results", "")}',
...
def main():
    ...
    if len(new_scopes_subs) > 0:
        ...
        if args.ports:
            ...
            new_port_probes = db_get_modified(port_probes, db['ports'], ['host', 'port'],
                                              ['host', 'ip', 'port', 'scope'], compare.port)
